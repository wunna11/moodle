...
/**
 * Deactivate or reactivate a lecturer. Employees are never hard-deleted
 * since attendance/leave/payroll history references them; deactivation
 * (employmentstatus = terminated) preserves that history.
 *
...
if (!$lecturer) {
    throw new moodle_exception('invalidrecord', 'error', new moodle_url('/local/hrdepartment/lecturer/index.php'));
}

$returnurl = new moodle_url('/local/hrdepartment/lecturer/view.php', ['id' => $id]);
$actionurl = new moodle_url('/local/hrdepartment/lecturer/delete.php', ['id' => $id, 'reactivate' => $reactivate]);
...
if ($confirm && confirm_sesskey()) {
    ...
    $syncresults = lecturer_manager::set_employment_status($id, $newstatus, $USER->id);

    $message = $reactivate
        ? get_string('lecturerreactivated', 'local_hrdepartment')
        : get_string('lecturerdeactivated', 'local_hrdepartment');

    $failed = count(array_filter($syncresults, static fn($r) => !$r['ok']));
    if ($failed > 0) {
        $message .= ' ' . get_string('assignmentsyncwarning', 'local_hrdepartment', (object) [
            'ok' => count($syncresults) - $failed,
...
$confirmstring = $reactivate
    ...
    : get_string('confirmdeactivate', 'local_hrdepartment', $lecturer->fullname);
