...
$canmanage = has_capability('local/hrdepartment:managestudentleave', $context);
if (!$canmanage) {
    require_capability('local/hrdepartment:viewstudentleave', $context);
}
...
echo $OUTPUT->heading(get_string('leavebalances', 'local_hrdepartment'));
...
if ($search !== '' && !$studentid) {
    ...
    if (empty($matches)) {
        echo $OUTPUT->notification(get_string('nostudentsfound', 'local_hrdepartment'), 'info');
    } else {
...
if ($studentid) {
    ...
    echo html_writer::tag('h4', fullname($student) . ' (' . $student->email . ')');
    ...
    if (empty($balances)) {
        echo $OUTPUT->notification(get_string('nobalancesforstudent', 'local_hrdepartment'), 'info');
    } else {
        echo html_writer::table($table);
    }
} else if ($search === '') {
    echo $OUTPUT->notification(get_string('pickastudent', 'local_hrdepartment'), 'info');
}
