...
//    $DB->insert_record("koperedashboard_req_message", $koperedashboardreqmessage);
    ...
    $msgtext = is_array($data->message_editor) ? $data->message_editor["text"] : $data->message_editor;
    $msgformat = is_array($data->message_editor) ? ($data->message_editor["format"] ?? FORMAT_HTML) : FORMAT_HTML;
...
// Kopere Dashboard Layout ကို အသုံးပြု၍ Form ကို မှန်ကန်စွာ Render လုပ်ခြင်း
ob_start();
echo \html_writer::start_div('container-fluid py-4');
echo \html_writer::tag('h2', get_string("new_title", "koperedashboard_requests"), ['class' => 'mb-4']);
$form->display();
