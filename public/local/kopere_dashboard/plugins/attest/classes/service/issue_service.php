...
public static function render_html(string $htmltemplate, array $data): string {
        // Moodle Core's Mustache Engine Class (Moodle 4.x Compatible)
        if (class_exists('\core\output\mustache_engine')) {
            $engine = new \core\output\mustache_engine([
                "escape" => function($value) {
                    return htmlspecialchars($value, ENT_QUOTES | ENT_SUBSTITUTE, "UTF-8");
                },
            ]);
        ...
        // Fallback for older Moodle or custom Mustache setups
        ...
        // Standard Moodle Output Render Fallback
        global $OUTPUT;
        return $OUTPUT->render_from_template('core/stubs', $data);
    }
