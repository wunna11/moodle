...
/**
 * Deactivate or reactivate a scholarship definition. Never hard-deleted,
 * since scholarship requests reference it - deactivating just stops it
...
$returnurl = new moodle_url('/local/financedepartment/pages/scholarships/view.php', ['id' => $id]);
$actionurl = new moodle_url('/local/financedepartment/pages/scholarships/deactivate.php', ['id' => $id, 'reactivate' => $reactivate]);
...
if ($confirm && confirm_sesskey()) {
    ...
    scholarship_manager::set_status($id, $newstatus, $USER->id);

    $message = $reactivate
        ? get_string('scholarshipreactivated', 'local_financedepartment')
        : get_string('scholarshipdeactivated', 'local_financedepartment');
...
$confirmstring = $reactivate
    ? get_string('confirmreactivatescholarship', 'local_financedepartment', format_string($scholarship->name))
    : get_string('confirmdeactivatescholarship', 'local_financedepartment', format_string($scholarship->name));
